// Auto-generated feature route: rights-clearance-agent
// Domain: agentic rights clearance agent — Capability: traversing samples and rights holders
// Mounted under: /api/rights-clearance-agent
...
// POST /api/rights-clearance-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic rights clearance agent. Your task is traversing samples and rights holders. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/rights-clearance-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/rights-clearance-agent/:id — fetch one
...
// DELETE /api/rights-clearance-agent/:id — discard a run
