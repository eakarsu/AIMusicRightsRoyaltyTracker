...
// Route: /api/gap-licensing-recommender
// Feature: Licensing Recommender
// Description: (best licensing path per asset)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Licensing Recommender. (best licensing path per asset)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Licensing Recommender\nDescription: (best licensing path per asset)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['licensing-recommender', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'licensing-recommender', title: 'Licensing Recommender', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'licensing-recommender', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['licensing-recommender']);
      return res.json({ slug: 'licensing-recommender', items: r.rows || [] });
    }
    return res.json({ slug: 'licensing-recommender', items: [] });
  } catch (e) {
    return res.json({ slug: 'licensing-recommender', items: [], error: (e && e.message) || 'Server error' });
  }
});
