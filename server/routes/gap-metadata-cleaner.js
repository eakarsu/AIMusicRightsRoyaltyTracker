...
// Route: /api/gap-metadata-cleaner
// Feature: Metadata Cleaner
// Description: (standardize artist names, track titles)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Metadata Cleaner. (standardize artist names, track titles)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Metadata Cleaner\nDescription: (standardize artist names, track titles)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['metadata-cleaner', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'metadata-cleaner', title: 'Metadata Cleaner', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'metadata-cleaner', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['metadata-cleaner']);
      return res.json({ slug: 'metadata-cleaner', items: r.rows || [] });
    }
    return res.json({ slug: 'metadata-cleaner', items: [] });
  } catch (e) {
    return res.json({ slug: 'metadata-cleaner', items: [], error: (e && e.message) || 'Server error' });
  }
});
