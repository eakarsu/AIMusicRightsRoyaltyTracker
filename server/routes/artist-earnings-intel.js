// Auto-generated feature route: artist-earnings-intel
// Domain: artist earnings intelligence — Capability: benchmarking earnings per genre
// Mounted under: /api/artist-earnings-intel
...
// POST /api/artist-earnings-intel/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a artist earnings intelligence. Your task is benchmarking earnings per genre. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/artist-earnings-intel/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/artist-earnings-intel/:id — fetch one
...
// DELETE /api/artist-earnings-intel/:id — discard a run
