...
// Route: /api/gap-performance
// Feature: Performance
// Description: Performance royalty tracking (PROs like ASCAP, BMI)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Performance. Performance royalty tracking (PROs like ASCAP, BMI)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'performance', title: 'Performance', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'performance', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['performance']);
      return res.json({ slug: 'performance', items: r.rows || [] });
    }
    return res.json({ slug: 'performance', items: [] });
  } catch (e) {
    return res.json({ slug: 'performance', items: [], error: (e && e.message) || 'Server error' });
  }
});
