// Auto-generated feature route: catalog-acquisition-advisor
// Domain: catalog acquisition advisor — Capability: producing investment theses
// Mounted under: /api/catalog-acquisition-advisor
...
// POST /api/catalog-acquisition-advisor/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a catalog acquisition advisor. Your task is producing investment theses. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'catalog-acquisition-advisor',
      input: payload,
      output: parsed,
...
// GET /api/catalog-acquisition-advisor/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/catalog-acquisition-advisor/:id — fetch one
...
// DELETE /api/catalog-acquisition-advisor/:id — discard a run
